...
function buildEdges(entries) {
  ...
  for (const cat of Object.keys(byCategory)) {
    ...
    for (let i = 0; i < group.length; i++) {
      triCache[i] = trigrams(
        tokenize(group[i].content || group[i].summary || '')
      );
    }
...
function init() {
  ...
  if (graphState && graphState.nodeCount === deduped.length) {
    ...
    if (age < 60000) {
      return {
        nodes:
          graphState.nodeCount || Object.keys(graphState.nodes || {}).length,
...
function getContext(prompt) {
  ...
  for (let i = 0; i < topEntries.length; i++) {
    ...
    lines.push(
      `  * (${e.score.toFixed(2)}) ${display} [rank #${i + 1}, ${accessed}x accessed]`
    );
  }
  ...
  for (const entry of ranked.entries) {
    if (ids.includes(entry.id)) {
      entry.confidence = Math.max(
        0,
        Math.min(1, (entry.confidence || 0.5) + amount)
      );
...
function consolidate() {
  ...
  if (!store || !Array.isArray(store)) {
    return {
      ...
      edges: 0,
      newEntries: 0,
      message: 'No store to consolidate'
    };
  }
  ...
  if (graph && graph.nodes) {
    ...
    for (const id of Object.keys(graph.nodes)) {
      ...
      const hoursSinceCreation =
        (now - (node.createdAt || now)) / (1000 * 60 * 60);
  ...
  if (newEntries > 0 || store.length < preDedupCount)
    writeJSON(STORE_PATH, store);
...
function stats(outputJson) {
  ...
  const confMean = confidences.length
    ...
    : 0;
  const confMedian = confidences.length
  ...
  const topPatterns = ((ranked && ranked.entries) || [])
    .slice(0, 10)
    .map((e, i) => ({
      rank: i + 1,
      summary: (e.summary || '').slice(0, 60),
  ...
  if (graph && graph.edges) {
    for (const e of graph.edges) {
      edgeTypes[e.type || 'unknown'] =
        (edgeTypes[e.type || 'unknown'] || 0) + 1;
    }
  }
  ...
  if (trend) {
    ...
    console.log(
      `    Node growth:       ${trend.nodeGrowth >= 0 ? '+' : ''}${trend.nodeGrowth}`
    );
    console.log(
      `    Edge growth:       ${trend.edgeGrowth >= 0 ? '+' : ''}${trend.edgeGrowth}`
    );
    console.log(
  ...
  if (!delta && !trend) {
    console.log(
      '  No history yet — run more sessions to see deltas and trends.'
    );
    console.log('');
  }
