...
class JsonFileBackend {
  ...
  async query(opts) {
    ...
    if (opts?.namespace)
      results = results.filter((e) => e.namespace === opts.namespace);
...
async function doImport() {
  ...
  try {
    ...
    dim(
      `├─ Learning: ${config.learningBridge.enabled ? 'active' : 'disabled'}`
    );
    dim(`├─ Graph: ${config.memoryGraph.enabled ? 'active' : 'disabled'}`);
    dim(
      `└─ Agent scopes: ${config.agentScopes.enabled ? 'active' : 'disabled'}`
    );
  } catch (err) {
...
async function doSync() {
  ...
  try {
    ...
    dim(
      `├─ Categories updated: ${syncResult.categories?.join(', ') || 'none'}`
    );
...
async function doStatus() {
  ...
  console.log(
    `  LearningBridge: ${config.learningBridge.enabled ? '✅ Enabled' : '⏸ Disabled'}`
  );
  console.log(
    `  MemoryGraph:    ${config.memoryGraph.enabled ? '✅ Enabled' : '⏸ Disabled'}`
  );
  console.log(
    `  AgentScopes:    ${config.agentScopes.enabled ? '✅ Enabled' : '⏸ Disabled'}`
  );
