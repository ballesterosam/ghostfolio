...
const CONFIG = {
  ...
  hideCost: /^(1|true|yes|on)$/i.test(
    process.env.RUFLO_STATUSLINE_HIDE_COST || ''
  )
};
  ...
  try {
    ...
    try {
      ...
      for (const prefix of [
        ...
        if (prefix)
          globalModuleDirs.push(path.join(prefix, 'lib', 'node_modules'));
      }
  ...
  try {
    ...
    const cmd = cliBin
      ? '"' +
        ...
        '" "' +
        cliBin +
        '" hooks statusline --json 2>/dev/null'
...
function getLocalAgentDB() {
  ...
  try {
    ...
    for (const f of ['.swarm/memory.db', 'ruvector.db']) {
      try {
        bytes += fs.statSync(path.join(CWD, f)).size;
      } catch {
        /* missing */
      }
    }
    ...
    if (fs.existsSync(memDb)) {
      ...
      const sql =
        Q +
        'SELECT (SELECT COUNT(*) FROM memory_entries WHERE embedding IS NOT NULL)||' +
        "'|'" +
...
function getLocalIntegration() {
  const integration = {
    mcpServers: { enabled: 0, total: 0 },
    hasDatabase: false
  };
  try {
    for (const f of ['.swarm/memory.db', 'ruvector.db']) {
      if (fs.existsSync(path.join(CWD, f))) {
        integration.hasDatabase = true;
        break;
      }
    }
    const names = new Set();
    const projMcp = readJSON(path.join(CWD, '.mcp.json'));
    if (projMcp && projMcp.mcpServers)
      for (const k of Object.keys(projMcp.mcpServers)) names.add(k);
    const claudeJson = readJSON(path.join(os.homedir(), '.claude.json'));
    if (claudeJson) {
      if (claudeJson.mcpServers)
        for (const k of Object.keys(claudeJson.mcpServers)) names.add(k);
...
function getGitInfo() {
  ...
  const parts = raw.split('---SEP---').map(function (s) {
    return s.trim();
  });
  if (parts.length >= 4) {
        ...
        if (CWD === projectPath || CWD.startsWith(projectPath + '/')) {
          ...
          if (usage) {
            ...
            if (ids.length > 0) {
              ...
              for (const id of ids) {
                const ts =
                  usage[id] && usage[id].lastUsedAt
                    ? new Date(usage[id].lastUsedAt).getTime()
                    : 0;
                if (ts > latest) {
                  latest = ts;
                  modelId = id;
...
function getModelFromStdin() {
  ...
  return data && data.model && data.model.display_name
...
function generateStatusline() {
  ...
  const modelName =
    getModelFromStdin() || (d.user && d.user.modelName) || 'Claude Code';
  ...
  // Header
  let header =
    c.bold + c.brightPurple + '▊ RuFlo V' + pkgVersion + ' ' + c.reset;
  header +=
    (coordinationActive ? c.brightCyan : c.dim) +
    '● ' +
    c.brightCyan +
    git.name +
    c.reset;
  if (git.gitBranch) {
    ...
    if (changes > 0) {
      ...
      if (git.modified > 0)
        ind += c.brightYellow + '~' + git.modified + c.reset;
      if (git.untracked > 0) ind += c.dim + '?' + git.untracked + c.reset;
      header += ' ' + ind;
    }
    if (git.ahead > 0)
      header += ' ' + c.brightGreen + '↑' + git.ahead + c.reset;
    if (git.behind > 0)
      header += ' ' + c.brightRed + '↓' + git.behind + c.reset;
  }
  header +=
    '  ' + c.dim + '│' + c.reset + '  ' + c.purple + modelName + c.reset;
  const duration = costInfo ? costInfo.duration : '';
  if (duration)
    header +=
      '  ' + c.dim + '│' + c.reset + '  ' + c.cyan + '⏱ ' + duration + c.reset;
  ...
  if (hasHnsw && vectorCount > 0) {
    const speedup =
      vectorCount > 10000 ? '12500x' : vectorCount > 1000 ? '150x' : '10x';
    perfIndicator = c.brightGreen + '⚡ HNSW ' + speedup + c.reset;
  } else if (patternsLearned > 0) {
    const pk =
      patternsLearned >= 1000
        ? (patternsLearned / 1000).toFixed(1) + 'k'
  ...
  lines.push(
    c.brightCyan +
      ...
      c.reset +
      '    ' +
      progressBar(domainsCompleted, totalDomains) +
      '  ' +
      domainsColor +
      domainsCompleted +
      c.reset +
  ...
  const swarmInd = coordinationActive
  ...
  const secIcon =
    secStatus === 'CLEAN'
      ? '🟢'
      : secStatus === 'IN_PROGRESS' || secStatus === 'STALE'
        ? '🟡'
        : secStatus === 'NONE'
          ? '⚪'
          : '🔴';
  ...
  lines.push(
    c.brightYellow +
      ...
      ' [' +
      agentsColor +
      String(activeAgents).padStart(2) +
      c.reset +
      '/' +
      c.brightWhite +
      maxAgents +
      ...
      intellColor +
      '🧠 ' +
      String(intelligencePct).padStart(3) +
      '%' +
      c.reset
  );
  ...
  // Line 3: Architecture
  const dddColor =
    dddProgress >= 50 ? c.brightGreen : dddProgress > 0 ? c.yellow : c.red;
  const adrColor =
    adrCount > 0 ? (adrImpl === adrCount ? c.brightGreen : c.yellow) : c.dim;
  ...
  const sizeDisp =
    dbSizeKB >= 1024 ? (dbSizeKB / 1024).toFixed(1) + 'MB' : dbSizeKB + 'KB';
  ...
  if (mcpServers.total > 0) {
    const mcpCol =
      mcpServers.enabled === mcpServers.total
        ? c.brightGreen
        : mcpServers.enabled > 0
          ? c.brightYellow
          : c.red;
    integStr +=
      c.cyan +
...
function generateJSON() {
  ...
  return Object.assign({}, d, {
    user: Object.assign(
      { name: git.name, gitBranch: git.gitBranch },
      d.user || {}
    ),
    git: {
